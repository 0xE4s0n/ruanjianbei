...
package com.android.zxb.engine.emotionapp.utils;
...
public class EmotionUtils {
    ...
    public static final int EMOTION_MY_TEST_TYPE = 0x0002;//测试表情
    ...
    public static List<ArrayMap<String, Integer>> EMOTION_ALL_MAPS;

    static {
        EMPTY_MAP = new ArrayMap<>();
        EMOTION_CLASSIC_MAP = new ArrayMap<>();

        EMOTION_ALL_MAPS = new ArrayList<>();
        EMOTION_ALL_MAPS.add(EMOTION_CLASSIC_MAP);
        ...
        EMOTION_CLASSIC_MAP.put("[气球]", R.drawable.d_travel);
    }
    ...
    public static int getImgByName(int EmotionType, String imgName) {
        ...
        switch (EmotionType) {
            case EMOTION_CLASSIC_TYPE:
                ...
                break;
            default:
                for (ArrayMap<String, Integer> name : EMOTION_ALL_MAPS) {
                    for (String key : name.keySet()) {
                        if (key.equals(imgName)) {
                            integer = name.get(imgName);
                            return integer;
                        }
                    }
                }
